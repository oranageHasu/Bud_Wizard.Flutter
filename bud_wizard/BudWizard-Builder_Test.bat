...
docker-compose --project-name budwizard build --build-arg BASE_URL="https://159.89.127.191:9001/api/v1/"

echo Step 2) Tag the created Docker Images appropriately
docker tag danksolutions/budwizard registry.digitalocean.com/bud-wizard-image-repo/budwizard-test:1.0.5

echo Step 3) Push the Docker images we just created to Docker Hub
docker push registry.digitalocean.com/bud-wizard-image-repo/budwizard-test:1.0.5
